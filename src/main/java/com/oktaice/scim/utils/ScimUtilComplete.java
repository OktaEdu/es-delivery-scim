...
public class ScimUtilComplete {
    ...
    public static Map<String, Object> userToPayload(User user){
        ...
        //ADD SCIM SCHEMA
        String[] schemas = {SCHEMA_USER_CORE, SCHEMA_USER_ENTERPRISE, SCHEMA_USER_OKTAICE};
    ...
    public static Map<String, Object> usersToPayload(List<User> users, Optional<Integer> startIndex,
        ...
        for(User u : users){
            ...
            usersList.add(userToPayload(u));
        }
        returnValue.put(LIST_RESPONSE_RESOURCE_PLACEHOLDER, usersList);
    ...
    public static void validatePatchOp(@RequestBody Map<String, Object> payload) throws RuntimeException{
        if(payload.get(PATCHOP_PLACEHOLDER) == null){
            throw new RuntimeException("Payload must contain operations attribute.");
        }
        if(payload.get(SCHEMA_ATTRIBUTE) == null){
            throw new RuntimeException("Payload must contain schema attribute.");
        }
        List schema = (List)payload.get(SCHEMA_ATTRIBUTE);
        if(!schema.contains(SCHEMA_PATCHOP)){
            throw new RuntimeException("The 'schemas' type in this request is not supported.");
        }
    }//validatePatchOp
